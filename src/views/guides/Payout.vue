<template>
    <section v-if="user.stripe_account_id && user.charges_enabled" class="flx gap-40 main">
    ...
    <div v-else-if="!user.details_submitted" class="empty-state flx column jc-c ai-c gap-16">
        <div class="centered text-center">
            ...
            <h3>Details are under review</h3>
            <div class="mb-24">In order to accept payment and receive payouts, you will need setup a stripe connect account.</div>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        async fetchAccount() {
            try {
                const res = await axios.get(this.hostname+'/api/stripe?token='+ this.token)
