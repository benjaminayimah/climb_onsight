            ...
            <div class="search-result-section overflow-y-scroll scroll-hidden flx column">
                ...
                <div v-else class="centered empty">
                    <div class="flx column gap-4 ai-c">
                        ...
                        <h3>Not found</h3>
                        <div>Sorry, no event was found for the specified parameter</div>
                    </div>
                </div>
            </div>
    ...
    computed: {
        ...
        computedResults() {
            ...
            if(query.query || (query.lat && query.lng)) {
                ...
                result = this.events.filter(data => new Date(data.end_date) > new Date())
            }
            return result
        },
        computedFilters() {
            ...
            if (result.length && (query.filter_category || query.filter_date)) {
                if(query.filter_category && !query.filter_date) {
                    filtered = result.filter(item => item.category === query.filter_category);
                }else if(!query.filter_category && query.filter_date) {
                    filtered = result.filter(item => item.start_date === query.filter_date);
                }
                else if(query.filter_category && query.filter_date) {
                    filtered = result.filter(item => item.category === query.filter_category && item.start_date === query.filter_date);
                }
