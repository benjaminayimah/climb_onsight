    ...
    methods: {
        formatPhoneNumber() {
            ...
            if (numericPhoneNumber.length > 3) {
              // Introduce the first space after the first three digits
              const firstPart = numericPhoneNumber.slice(0, 3);
              const restPart = numericPhoneNumber.slice(3);
      
              // Check if the restPart has a length greater than 3 to introduce the second space
              ...
              // If the length is less than or equal to 3, don't introduce spaces
